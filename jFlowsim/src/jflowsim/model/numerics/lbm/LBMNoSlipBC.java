...
public class LBMNoSlipBC extends BoundaryCondition {

    public LBMNoSlipBC(LBMUniformGrid _grid, int _type) {
        this.myGrid = _grid;
        this.type = _type;
    }
    ...
    public void apply(){
        ...
        if (type == EAST) {
            for (int j = 0; j < myGrid.ny; j++) {
                ...
                myGrid.f[nodeIndex + LbEQ.NW] = myGrid.ftemp[nodeIndex + LbEQ.SE];
                myGrid.f[nodeIndex + LbEQ.SW] = myGrid.ftemp[nodeIndex + LbEQ.NE];
            }
        } else if (type == WEST) {
            for (int j = 0; j < myGrid.ny; j++) {
                ...
                myGrid.f[nodeIndex + LbEQ.NE] = myGrid.ftemp[nodeIndex + LbEQ.SW];
                myGrid.f[nodeIndex + LbEQ.SE] = myGrid.ftemp[nodeIndex + LbEQ.NW];
            }
        } else if (type == NORTH) {

...
public class LBMNavierStokesGrid extends LBMUniformGrid {
    ...
    protected void allocateMemory() {
        ...
        System.out.println("LBMNodefreeGrid::allocateMemory() nx:" + nx + " ny:" + ny + " - " + nx * ny);
    }

    public void init(int x, int y, double _p, double _vx, double _vy) {

        double[] feq = new double[9];

        LbEQ.getBGKEquilibrium( 3*_p, _vx / dv, _vy / dv, feq);

        for (int dir = 0; dir < 9; dir++) {
            
            f[(x + y * nx) * 9 + dir] = feq[dir];
            //System.out.println("offset " + (x + y *nx)*9 + dir);
            //System.out.println("f is " + f[(x + y * nx) * 9 + dir]);
            //System.out.println(f[52]);
            ftemp[(x + y * nx) * 9 + dir] = feq[dir];
            
        }
        //System.out.println("ftemp at 2" + ftemp[2]);
    }
